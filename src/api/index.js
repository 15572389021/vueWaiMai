...
// 2.获取 msite 页面食品分类列表
export const reqCategorys = () => fetch(BASE_URL + '/index_category');

// 3.获取 msite 商铺列表(根据query参数：经纬度)
// 将经纬度两个数据作为一个参数对象传入
// 也可以两个数据分别传入ajax， 然后再放入一个对象参数内， 如下面的手机号接口
export const reqShops = ({latitude, longitude}) => fetch(BASE_URL + '/shops', {latitude, longitude});
...
// 7.获取短信验证码
export const reqSendCode = (phone) => fetch(BASE_URL + '/sendcode', {phone});
